...
| **原语言Logo** | 自动获取原语言的Logo，提升媒体库视觉统一性 |
| **简繁海报** | **港/台/中** 电影统一请求 `zh` 语言海报（港澳台很多电影海报在TMDB并未被区分为zh-HK，zh-TW，故妥协。） |
| **语言映射** | 内置主流语言/地区映射表，支持 `en-US`, `fr-FR`, `ko-KR` 等 BCP 47 标准 |
| **用户自定义** | 可开启/关闭插件、Logo、测试模式 |
...
5.  在`媒体库中`去选中`OriginalPoster`，并置顶。根据Emby规则，那个刮削插件置顶，那个插件返回的海报优先。
